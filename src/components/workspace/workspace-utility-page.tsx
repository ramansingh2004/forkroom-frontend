...
} from "@/components/decision-room/evidence-preview";
import { IntegrationsPage } from "@/components/integrations/integrations-page";
import { MentionsPage } from "@/components/mentions/mentions-page";
...
export type WorkspaceUtilityKind =
  "documents" | "activity" | "mentions" | "integrations";
...
> = {
  documents: {
    ...
    title: "Documents",
    description:
      "Review evidence files in their decision context rather than as an unrelated file drive.",
  },
  activity: {
    eyebrow: "WORKSPACE / HISTORY",
    title: "Recent activity",
    description:
      "Follow decision, evidence, and attention events for this workspace in time order.",
  },
  mentions: {
    eyebrow: "WORKSPACE / CONVERSATION",
    title: "Mentions",
    description:
      "Find comments where a workspace member explicitly requested your attention.",
  },
  integrations: {
    eyebrow: "WORKSPACE / SYSTEM",
    title: "Integrations",
    description:
      "Connect external systems without mixing configuration into daily decision work.",
  },
};
...
export function WorkspaceUtilityPage({ workspaceId, kind }: Props) {
  ...
  return (
    <div className={styles.page}>
      ...
      {kind === "mentions" && <MentionsPage workspaceId={workspaceId} />}
      {kind === "integrations" && (
...
function DocumentsView({ workspaceId }: { workspaceId: string }) {
  ...
  const decisionById = useMemo(
    () =>
      new Map(
        (decisions.data ?? []).map((decision) => [decision.id, decision]),
      ),
    [decisions.data],
  );
    ...
    try {
      ...
      notifications.show({
        ...
        message: getApiErrorMessage(
          error,
          "ForkRoom could not create a fresh authorized file link.",
        ),
      });
    } finally {
      setActiveId(null);
  ...
  if (preview)
    return <EvidencePreview item={preview} onBack={() => setPreview(null)} />;
  if (attachments.isPending || decisions.isPending || members.isPending)
  ...
  const items = (attachments.data ?? []).filter(
  ...
  if (items.length === 0) {
    return (
      <EmptyState
        icon={IconFileText}
        title="No evidence documents yet"
        description="Files added from a Decision Room will appear here with their decision and proposal context."
      />
    );
  }
        ...
        return (
          <article className={styles.documentRow} key={attachment.id}>
            <div>
              ...
              <span>
                {attachment.media_type} · {formatBytes(attachment.size_bytes)}
              </span>
            </div>
            <div>
              {decision ? (
                <Link href={`/w/${workspaceId}/decisions/${decision.id}`}>
                  {decision.title}
                </Link>
              ) : (
...
function ActivityView({ workspaceId }: { workspaceId: string }) {
  ...
  if (
    ...
    notificationFeed.isPending
  )
    return <UtilitySkeleton />;
  if (decisions.isError || attachments.isError || notificationFeed.isError) {
    const error =
      decisions.error ?? attachments.error ?? notificationFeed.error;
    return (
      <UtilityError
  ...
  const events = [
    ...(decisions.data ?? []).map((decision) => ({
      ...
      detail: `Decision is ${decision.status}.`,
    ...
    ...(attachments.data ?? [])
      ...
      .map((item) => ({
        ...
        at: item.updated_at,
        title: item.filename,
        detail: `Evidence is ${item.status}.`,
        href: item.decision_id
          ? `/w/${workspaceId}/decisions/${item.decision_id}`
  ...
  if (events.length === 0)
    return (
      <EmptyState
        ...
        description="Decision changes, evidence processing, and attention events will appear here."
      />
    );
  return (
    <section className={styles.timeline}>
      {events.map((event) => (
        <article key={event.id} className={styles.activityRow}>
...
function UtilitySkeleton() {
  return (
    <div className={styles.skeleton} aria-label="Loading page">
      <Skeleton height={72} />
      <Skeleton height={72} />
      <Skeleton height={72} />
    </div>
...
function UtilityError({
  ...
  return (
    <Alert color="red" title={title}>
      {getApiErrorMessage(
        error,
        "ForkRoom could not load this workspace surface.",
      )}
      <Button
        mt="sm"
        size="compact-sm"
